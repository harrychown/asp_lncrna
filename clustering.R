library(ggplot2) # version 1.2.0
library(venn) # version 1.10
library(UpSetR) # version 1.4.0
library(viridis) # version 0.4.0
### PERFORM CLUSTERING ###
iterative_kmeans <- function(input_data, k_val, iter_num){
  kmeans_results <- c()
  kmeans_withinss <- c()
  for(i in 1:iter_num){
    set.seed(i)
    km_out <-  kmeans(input_data, k_val, iter.max = 1000, nstart = 5*k_val)
...
cluster_data <- function(input_data, k_val, treeheight, fileprefix, iterval){
  library(pheatmap)
  #set.seed(0)
  ...
  kmeans_res <- iterative_kmeans(input_data, k_val, iterval)
...
multi_graph <- function(input_data, fileprefix){
  ...
  write.csv(MULTIlong_data,paste(c(fileprefix, ".longdata.csv"), sep="", collapse="") )
  # Plot clusters
...
multi_shared <- function(input_data, fileprefix){
  ...
  png(paste(c(fileprefix,".upset.png"), sep ="", collapse=""), width = 4000, height = 3000, pointsize = 20, res = 300)
  print(upset_p)
  dev.off()
  upset_p$x1 <- x1
  return(upset_p)
...
itra_graph <- function(input_data, inverse_clusters, fileprefix){
  ...
  write.csv(long_data,paste(c(fileprefix, ".longdata.csv"), sep="", collapse="") )
  # Plot clusters
